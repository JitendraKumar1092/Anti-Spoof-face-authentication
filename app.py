...
class App:
    def __init__(self):
        ...
        self.login_status = {}  # Dictionary to keep track of login status
    ...
    def handle_authentication(self, action):
        ...
        if label == 1:
            name = util.recognize(self.most_recent_capture_arr, self.db_dir, tolerance=0.5)  # Adjust tolerance here
            if name in ['unknown_person', 'no_persons_found']:
                util.msg_box('Ups...', 'Unknown user. Please register new user or try again.')
            else:
                if action == 'in':
                    if name in self.login_status and self.login_status[name]:
                        util.msg_box('Already logged in', f'{name} is already logged in.')
                    else:
                        self.login_status[name] = True
                        util.msg_box('Login successful!', f'Welcome, {name}.')
                        with open(self.log_path, 'a') as f:
                            f.write(f'{name},{datetime.datetime.now()},{action}\n')
                elif action == 'out':
                    if name in self.login_status and self.login_status[name]:
                        self.login_status[name] = False
                        util.msg_box('Logout successful!', f'It was great to see you, {name}!')
                        with open(self.log_path, 'a') as f:
                            f.write(f'{name},{datetime.datetime.now()},{action}\n')
                    else:
                        util.msg_box('Not logged in', f'{name} is not currently logged in.')
        else:
